          ...
          <div className="text-center">
              ...
              <div className="relative flex flex-col" ref={testimonialsRef}>
                {slicedTestimonials.map((item: any, index: number) => (
                  <Transition
                    ...
                    beforeEnter={() => heightFix()}
                  >
                    <div className="text-base md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary/60 via-primary to-primary/60">
                      {item.text}
                    </div>
                  </Transition>
                ))}
            ...
            <div className="flex flex-wrap justify-center -m-1.5 px-8 sm:px-6">
              {slicedTestimonials.map((item: any, index: number) => (
                <button
                  className={cn(
                    `px-2 py-1 rounded-full m-1.5 text-xs border border-transparent text-primary-light transition duration-150 ease-in-out [background:linear-gradient(theme(colors.primary),_theme(colors.primary))_padding-box,_conic-gradient(theme(colors.accent),_theme(colors.accent-light)_25%,_theme(colors.accent-light)_75%,_theme(colors.accent)_100%)_border-box] relative before:absolute before:inset-0 before:bg-accent/30 before:rounded-full before:pointer-events-none ${active === index
                      ? "border-accent/50"
                ...
                >
                  <span className="relative">
                    ...
                    <span className="text-primary-light hidden sm:inline-block">
                      -
                    </span>{" "}
                    <span className="hidden sm:inline-block text-primary-light">
                      {item.user.job}
                    </span>
                  </span>
                </button>
