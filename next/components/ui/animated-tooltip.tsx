          ...
          <AnimatePresence mode="popLayout">
            {hoveredIndex === item.id && (
              ...
              >
                ...
                <div className="absolute left-10 w-[40%] z-30 -bottom-px bg-gradient-to-r from-transparent via-primary-light to-transparent h-px" />
                <div className="font-bold text-primary relative z-30 text-base">
                  {`${item.firstname} ${item.lastname}`}
                </div>
                <div className="text-primary-light text-xs">{item.job}</div>
              </motion.div>
            )}
          </AnimatePresence>
          <Image
